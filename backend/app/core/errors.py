from __future__ import annotations

from dataclasses import dataclass, field
from datetime import UTC, datetime
from typing import Any
from uuid import uuid4

from fastapi.encoders import jsonable_encoder

from app.schemas.errors import ErrorResponse, ErrorSeverity


@dataclass(frozen=True)
class ErrorDefinition:
    title: str
    message: str
    severity: ErrorSeverity = "error"
    source: str = "backend"
    possible_causes: list[str] = field(default_factory=list)
    recovery_actions: list[str] = field(default_factory=list)


ERROR_DEFINITIONS: dict[str, ErrorDefinition] = {
    "CURRENT_MARKET_MISSING": ErrorDefinition(
        title="Current market is not available",
        message="The bot has not discovered the active BTC Up/Down market yet.",
        severity="warning",
        source="market_discovery",
        possible_causes=["Celery market discovery has not run yet", "Gamma API is unavailable"],
        recovery_actions=["Start celery beat and worker", "Call GET /api/markets/current", "Check Gamma API logs"],
    ),
    "UP_ORDERBOOK_MISSING": ErrorDefinition(
        title="UP orderbook is missing",
        message="The strategy cannot evaluate because no UP orderbook snapshot has been recorded.",
        severity="warning",
        source="clob_orderbook",
        possible_causes=["Orderbook polling has not run", "Market WebSocket is disconnected"],
        recovery_actions=["Start Celery orderbook polling", "Check CLOB API connectivity"],
    ),
    "DOWN_ORDERBOOK_MISSING": ErrorDefinition(
        title="DOWN orderbook is missing",
        message="The strategy cannot evaluate because no DOWN orderbook snapshot has been recorded.",
        severity="warning",
        source="clob_orderbook",
        possible_causes=["Orderbook polling has not run", "Market WebSocket is disconnected"],
        recovery_actions=["Start Celery orderbook polling", "Check CLOB API connectivity"],
    ),
    "CURRENT_CHAINLINK_TICK_MISSING": ErrorDefinition(
        title="BTC live price is not available",
        message="The strategy cannot evaluate the market because no current BTC/USD Chainlink tick has been recorded.",
        severity="warning",
        source="rtds_chainlink",
        possible_causes=[
            "realtime_runner is not running",
            "RTDS WebSocket is disconnected",
            "RTDS subscription format is invalid",
            "Redis/dashboard event pipeline is not running",
        ],
        recovery_actions=[
            "Start python -m app.workers.realtime_runner",
            "Verify POLYMARKET_RTDS_WSS",
            "Check backend logs for RTDS reconnect errors",
            "Wait until a btc/usd tick is persisted",
        ],
    ),
    "START_CHAINLINK_TICK_MISSING": ErrorDefinition(
        title="BTC start price is not available",
        message="The strategy needs a BTC/USD tick near market start to calculate direction.",
        severity="warning",
        source="rtds_chainlink",
        possible_causes=["realtime_runner started after the market began", "RTDS ticks were not persisted"],
        recovery_actions=["Keep realtime_runner running before the next cycle starts", "Check Chainlink tick persistence"],
    ),
    "MARKET_DATA_STALE": ErrorDefinition(
        title="Market data is stale",
        message="The latest market orderbook data is older than the configured freshness limit.",
        severity="warning",
        source="clob_orderbook",
        possible_causes=["Market WebSocket disconnected", "Orderbook polling is delayed"],
        recovery_actions=["Check market_ws_status", "Restart realtime_runner or Celery worker"],
    ),
    "CHAINLINK_DATA_STALE": ErrorDefinition(
        title="BTC price data is stale",
        message="The latest BTC/USD Chainlink tick is older than the configured freshness limit.",
        severity="warning",
        source="rtds_chainlink",
        possible_causes=["RTDS WebSocket disconnected", "No recent btc/usd update received"],
        recovery_actions=["Check rtds_status", "Restart realtime_runner"],
    ),
    "STRATEGY_CONTEXT_INCOMPLETE": ErrorDefinition(
        title="Strategy context is incomplete",
        message="The strategy skipped evaluation because required market, orderbook, or BTC data is missing.",
        severity="warning",
        source="strategy",
        possible_causes=["Market discovery or tick ingestion has not completed"],
        recovery_actions=["Review the listed missing inputs", "Wait for fresh orderbook and Chainlink data"],
    ),
    "MARKET_WS_DISCONNECTED": ErrorDefinition(
        title="Market WebSocket is disconnected",
        message="Live UP/DOWN market ticks are not currently streaming from Polymarket.",
        severity="warning",
        source="clob_market_ws",
        recovery_actions=["Start or restart realtime_runner", "Verify POLYMARKET_MARKET_WSS"],
    ),
    "RTDS_WS_DISCONNECTED": ErrorDefinition(
        title="RTDS WebSocket is disconnected",
        message="Realtime worker is not running or RTDS has not produced BTC/USD ticks yet.",
        severity="warning",
        source="rtds_chainlink",
        recovery_actions=["Start python -m app.workers.realtime_runner", "Check RTDS reconnect logs"],
    ),
    "POLYMARKET_CLOB_HTTP_ERROR": ErrorDefinition(
        title="Polymarket CLOB request failed",
        message="The backend could not fetch CLOB market data from Polymarket.",
        severity="error",
        source="polymarket_clob",
        recovery_actions=["Check Polymarket CLOB status", "Retry after a short delay"],
    ),
    "POLYMARKET_CLOB_TIMEOUT": ErrorDefinition(
        title="Polymarket CLOB did not respond in time",
        message="The backend could not fetch the latest orderbook from Polymarket before the timeout. This is usually a temporary network or upstream latency issue.",
        severity="warning",
        source="polymarket_clob",
        possible_causes=[
            "Polymarket CLOB API is slow",
            "Network latency from local machine/server",
            "Too many frequent orderbook requests",
            "No retry/backoff was previously configured",
        ],
        recovery_actions=[
            "Wait and retry",
            "Verify internet access from the backend runtime",
            "Increase POLYMARKET_HTTP_READ_TIMEOUT",
            "Use cached orderbook snapshots when available",
            "Prefer WebSocket market data for realtime UI",
        ],
    ),
    "POLYMARKET_CLOB_NETWORK_ERROR": ErrorDefinition(
        title="Polymarket CLOB network request failed",
        message="The backend could not reach Polymarket CLOB.",
        severity="warning",
        source="polymarket_clob",
        recovery_actions=["Verify internet access from the backend runtime", "Wait and retry"],
    ),
    "POLYMARKET_GAMMA_HTTP_ERROR": ErrorDefinition(
        title="Polymarket Gamma request failed",
        message="The backend could not fetch market metadata from Polymarket Gamma.",
        severity="error",
        source="polymarket_gamma",
        recovery_actions=["Check Polymarket Gamma status", "Retry market discovery"],
    ),
    "POLYMARKET_GAMMA_TIMEOUT": ErrorDefinition(
        title="Polymarket Gamma did not respond in time",
        message="The backend could not fetch market metadata from Polymarket Gamma before the timeout.",
        severity="warning",
        source="polymarket_gamma",
        recovery_actions=["Wait and retry", "Verify internet access from the backend runtime", "Increase POLYMARKET_HTTP_READ_TIMEOUT"],
    ),
    "POLYMARKET_GAMMA_NETWORK_ERROR": ErrorDefinition(
        title="Polymarket Gamma network request failed",
        message="The backend could not reach Polymarket Gamma.",
        severity="warning",
        source="polymarket_gamma",
        recovery_actions=["Verify internet access from the backend runtime", "Wait and retry"],
    ),
    "POLYMARKET_INVALID_RESPONSE": ErrorDefinition(
        title="Polymarket returned an invalid response",
        message="The backend could not parse a Polymarket response as valid JSON.",
        severity="error",
        source="polymarket",
        recovery_actions=["Check backend logs for response details", "Retry after a short delay"],
    ),
    "WALLET_PRIVATE_KEY_INVALID": ErrorDefinition(
        title="Wallet private key is invalid",
        message="The provided private key is not a valid 0x-prefixed 32-byte hex private key.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Verify the private key starts with 0x", "Use a dedicated Polygon trading wallet"],
    ),
    "WALLET_CONFIG_MISSING": ErrorDefinition(
        title="Wallet is not configured",
        message="Real trading requires a configured encrypted wallet credential.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Open the Wallet page and configure a dedicated trading wallet"],
    ),
    "WALLET_TABLE_MISSING": ErrorDefinition(
        title="Wallet credentials table is missing",
        message="Wallet credentials table does not exist. Run database migrations.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=[
            "Run: alembic upgrade head",
            "For local SQLite, make sure DATABASE_URL points to the expected database file.",
            "Restart backend after migration.",
        ],
    ),
    "WALLET_API_CREDENTIALS_MISSING": ErrorDefinition(
        title="Polymarket API credentials are missing",
        message="The wallet is configured, but Polymarket API credentials have not been created or derived yet.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Click Re-derive API Credentials on the Wallet page", "Verify Polymarket CLOB is reachable"],
    ),
    "WALLET_API_CREDENTIAL_DERIVATION_FAILED": ErrorDefinition(
        title="Could not derive Polymarket API credentials",
        message="Polymarket did not return usable API credentials.",
        severity="error",
        source="wallet_credentials",
        possible_causes=[
            "Polymarket rejected creating a new API key",
            "Wallet already has credentials and derive failed",
            "Private key/account is not accepted by Polymarket",
            "Geographic or account restrictions",
            "SDK version mismatch",
        ],
        recovery_actions=[
            "Verify the private key starts with 0x",
            "Use Polygon chain id 137",
            "Retry after confirming Polymarket CLOB is reachable",
        ],
    ),
    "WALLET_API_CREDENTIAL_RESPONSE_INVALID": ErrorDefinition(
        title="Polymarket returned incomplete API credentials",
        message="Polymarket responded to API credential derivation, but the response did not include all required credential fields.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Retry deriving API credentials", "Check Polymarket SDK compatibility"],
    ),
    "WALLET_ENCRYPTION_KEY_MISSING": ErrorDefinition(
        title="Credential encryption key is missing",
        message="CREDENTIAL_ENCRYPTION_KEY must be configured before wallet secrets can be stored or read.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Generate a Fernet key", "Set CREDENTIAL_ENCRYPTION_KEY in the backend environment"],
    ),
    "WALLET_DECRYPTION_FAILED": ErrorDefinition(
        title="Wallet credentials could not be decrypted",
        message="The stored wallet credentials could not be decrypted with the configured encryption key.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Verify CREDENTIAL_ENCRYPTION_KEY matches the key used to save the wallet"],
    ),
    "WALLET_CHAIN_ID_INVALID": ErrorDefinition(
        title="Wallet chain id is invalid",
        message="Wallet trading must use Polygon chain id 137.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Set chain_id to 137"],
    ),
    "WALLET_FUNDER_REQUIRED": ErrorDefinition(
        title="Funder address is required",
        message="Signature type 3 requires a funder or deposit wallet before real trading can be enabled.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Add the Polymarket funder/deposit wallet address on the Wallet page"],
    ),
    "POLYMARKET_AUTH_FAILED": ErrorDefinition(
        title="Polymarket authentication failed",
        message="Polymarket rejected the wallet or API credentials.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Re-derive API credentials", "Verify wallet signature type and funder address"],
    ),
    "POLYMARKET_SDK_MISSING": ErrorDefinition(
        title="Polymarket SDK is missing",
        message="py_clob_client_v2 is not installed or cannot be imported.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Install py_clob_client_v2 in the backend environment", "Restart the backend"],
    ),
    "POLYMARKET_INVALID_SIGNATURE": ErrorDefinition(
        title="Polymarket rejected the wallet signature",
        message="Polymarket rejected the L1 signature. The private key may be invalid or not formatted correctly.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Verify the private key", "Use a dedicated Polygon trading wallet", "Retry API credential derivation"],
    ),
    "POLYMARKET_AUTH_NETWORK_ERROR": ErrorDefinition(
        title="Polymarket authentication request failed",
        message="The backend could not reach Polymarket while deriving API credentials.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Verify backend internet access", "Retry when Polymarket CLOB is reachable"],
    ),
    "POLYMARKET_AUTH_REJECTED": ErrorDefinition(
        title="Polymarket rejected authentication",
        message="Polymarket rejected the authentication request.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Verify wallet address and private key", "Check whether this wallet can access Polymarket"],
    ),
    "POLYMARKET_GEO_BLOCKED": ErrorDefinition(
        title="Polymarket access is geo-blocked",
        message="Polymarket is not available from the current backend network location.",
        severity="error",
        source="wallet_credentials",
        recovery_actions=["Run the backend from a supported location", "Do not attempt real trading while blocked"],
    ),
    "POLYMARKET_CREDENTIALS_TEST_FAILED": ErrorDefinition(
        title="Wallet credential test failed",
        message="The backend could not validate the configured wallet credentials.",
        severity="warning",
        source="wallet_credentials",
        recovery_actions=["Re-derive API credentials", "Check backend logs for the safe error code"],
    ),
    "AUTH_REQUIRED": ErrorDefinition(
        title="Login required",
        message="Please log in to continue.",
        severity="warning",
        source="auth",
        recovery_actions=["Open the Login page", "Log in again if the session expired"],
    ),
    "INVALID_CREDENTIALS": ErrorDefinition(
        title="Invalid credentials",
        message="The username or password is incorrect.",
        severity="warning",
        source="auth",
        recovery_actions=["Check the username and password", "Ask an admin to reset your password"],
    ),
    "ACCOUNT_DISABLED": ErrorDefinition(
        title="Account disabled",
        message="This user account is disabled.",
        severity="warning",
        source="auth",
        recovery_actions=["Ask an admin to enable the account"],
    ),
    "ADMIN_REQUIRED": ErrorDefinition(
        title="Admin required",
        message="This action requires an admin user.",
        severity="warning",
        source="auth",
    ),
    "PERMISSION_DENIED": ErrorDefinition(
        title="Permission denied",
        message="You do not have permission to perform this action.",
        severity="warning",
        source="auth",
    ),
    "SESSION_EXPIRED": ErrorDefinition(
        title="Session expired",
        message="Your login session expired.",
        severity="warning",
        source="auth",
        recovery_actions=["Log in again"],
    ),
    "REAL_TRADING_NOT_READY": ErrorDefinition(
        title="Real trading is not ready",
        message="Real trading cannot be enabled until every readiness gate passes.",
        severity="warning",
        source="trading",
        recovery_actions=["Review trading readiness blockers", "Keep dry-run enabled until setup is complete"],
    ),
    "REAL_TRADING_CONFIRMATION_INVALID": ErrorDefinition(
        title="Confirmation phrase is invalid",
        message="Type ENABLE REAL TRADING exactly to enable real trading.",
        severity="warning",
        source="trading",
    ),
    "ORDERBOOK_PARSE_ERROR": ErrorDefinition(
        title="Orderbook response could not be parsed",
        message="Polymarket returned an orderbook payload the backend could not normalize.",
        severity="error",
        source="clob_orderbook",
        recovery_actions=["Check backend logs for the payload shape", "Update the orderbook parser"],
    ),
    "VALIDATION_ERROR": ErrorDefinition(
        title="Request validation failed",
        message="The request contains invalid or missing fields.",
        severity="warning",
        source="api",
    ),
    "HTTP_ERROR": ErrorDefinition(
        title="Request failed",
        message="The backend could not complete this request.",
        severity="error",
        source="api",
    ),
    "INTERNAL_SERVER_ERROR": ErrorDefinition(
        title="Unexpected backend error",
        message="An unexpected backend error occurred.",
        severity="error",
        source="backend",
        recovery_actions=["Check backend logs", "Retry after fixing the underlying error"],
    ),
}


class AppError(Exception):
    def __init__(self, code: str, *, technical_detail: str | None = None, status_code: int = 400) -> None:
        self.code = code
        self.technical_detail = technical_detail
        self.status_code = status_code
        super().__init__(technical_detail or code)


def build_error_response(
    code: str,
    *,
    technical_detail: str | None = None,
    request_id: str | None = None,
    overrides: dict[str, Any] | None = None,
) -> ErrorResponse:
    definition = ERROR_DEFINITIONS.get(code, ERROR_DEFINITIONS["INTERNAL_SERVER_ERROR"])
    payload = {
        "code": code,
        "title": definition.title,
        "message": definition.message,
        "severity": definition.severity,
        "source": definition.source,
        "possible_causes": list(definition.possible_causes),
        "recovery_actions": list(definition.recovery_actions),
        "technical_detail": technical_detail,
        "timestamp": datetime.now(UTC),
        "request_id": request_id or str(uuid4()),
    }
    if overrides:
        payload.update(overrides)
    return ErrorResponse(**payload)


def error_payload(code: str, *, technical_detail: str | None = None, request_id: str | None = None) -> dict[str, Any]:
    return jsonable_encoder(build_error_response(code, technical_detail=technical_detail, request_id=request_id))


def code_from_detail(detail: Any) -> str:
    if isinstance(detail, dict):
        value = detail.get("code")
        if isinstance(value, str):
            return value
    if isinstance(detail, str):
        normalized = detail.strip().upper()
        if normalized in ERROR_DEFINITIONS:
            return normalized
    return "HTTP_ERROR"
