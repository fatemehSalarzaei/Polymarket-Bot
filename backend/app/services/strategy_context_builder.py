...
class StrategyContextBuilder:
    async def build(self, session: AsyncSession, *, now: datetime | None = None) -> StrategyContextBuildResult:
        ...
        start_tick = await self._nearest_tick(session, market.start_ts)
        ...
        if latest_tick is None:
            missing.append("CURRENT_CHAINLINK_TICK_MISSING")
        if start_tick is None:
            missing.append("START_CHAINLINK_TICK_MISSING")
        if missing:
        ...
        chainlink_received_at = _ensure_aware(latest_tick.received_at)
        ...
        context = StrategyContext(
            ...
            btc_current_price=latest_tick.value,
            ...
            chainlink_data_age_seconds=_age_seconds(current_time, chainlink_received_at),
    ...
    async def _nearest_tick(self, session: AsyncSession, unix_ts: int | None) -> ChainlinkTick | None:
        if unix_ts is None:
